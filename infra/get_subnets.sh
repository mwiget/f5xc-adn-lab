...
terraform output -json | jq -r '.aws_eu_west_1.value[].aws_subnet[] | { subnet_name: .tags_all.Name, subnet_id: .id, vpc_id:.vpc_id, availability_zone: .availability_zone, security_group_id: .tags_all.security_group_id}' | jq -n '.subnets |= [inputs]' > subnets_eu_west_1.json
ls -l subnets*json
